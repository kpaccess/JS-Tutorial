...
import { getLessonsByCourse, getWeekLabel, PASS_THRESHOLD } from "@/lib/course-data";
...
export default async function CourseDashboardPage({
  ...
  const isJava = courseId === "java";
  const primaryColor = isJava ? "text-purple-400" : "text-yellow-400";
  const progressBg = isJava ? "bg-purple-500" : "bg-yellow-400";
  const buttonStyle = isJava
    ? "bg-purple-500/20 text-purple-300 hover:bg-purple-500/30"
    : "bg-yellow-400/20 text-yellow-400 hover:bg-yellow-400/30";
  const cardBorderHighlight = isJava ? "border-purple-500/30 bg-purple-500/5 hover:bg-purple-500/10" : "border-yellow-400/30 bg-yellow-400/5 hover:bg-yellow-400/10";
  const activeBadgeColor = isJava ? "bg-purple-500/20 text-purple-400 border-purple-500/30" : "bg-yellow-400/20 text-yellow-400 border-yellow-400/30";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">{courseTitle} Course 📚</h1>
          <p className="text-slate-400 mt-1">Sequential learning path. Complete each quiz to unlock the next day.</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center min-w-[160px]">
          <div className={`text-4xl font-black ${primaryColor}`}>{overallPct}%</div>
          ...
          { label: "Pass Rate", value: `${PASS_THRESHOLD}%`, total: null, icon: "✅" },
          {
            ...
            value: `${Math.round(
              (progressRows ?? [])
                ...
                .reduce((s, p) => s + (p.time_spent_minutes ?? 0), 0) / 60
            )}h`,
            total: null,
            icon: "⏱️",
          },
          { label: "Quizzes Passed", value: completedCount, total: lessons.length, icon: "🏆" },
        ].map(s => (
      ...
      {weeks.map(week => {
        ...
        return (
          <div key={week} className="space-y-3">
            <h2 className="text-xl font-bold text-white">{getWeekLabel(week, courseId)}</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {weekLessons.map((lesson, idx) => {
                ...
                // Lock lesson if previous not completed (except first global lesson)
                    ...
                    {!locked && (
                      <div className="space-y-2">
                        ...
                        <Progress
                          ...
                          className="h-1.5"
                        />
                        <Link href={`/dashboard/${courseId}/course/${lesson.id}`}>
                          <button className={`w-full text-sm py-2 px-3 rounded-lg font-medium transition-colors cursor-pointer ${
                            passed
